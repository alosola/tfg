...
one.T0 = 1757                     # inlet total temperature (exit combustor) = T4t [K]
one.P0 = 3263170                  # inlet total pressure (exit combustor) [Pa]
thr.P0 = 101305                    # outlet static pressure [Pa]
thr.T0 = 1710                  #  # outlet total temperature [K]
DeltaH_prod = 288.3e+06/622.79            #  # enthalpy produced by turbine [J/Kg]

# # DESIGN VARIABLES
mdot =  622.79                        # total mass flow [kg/s]
one.alpha = 0                     # stator inlet angle (0 because flow is axial) [rad]
GR = 0.5                         # reaction degree [-]
psi = 2.15                         # loading factor [-]
RHT = 0.97                      #  # ratio hub/tip radius
...
alpha2_init  = np.radians(65.5)     # stator angle [deg->rad]
...
h_c_rotor = 1                   # height/chord ratio rotor [-]
t_o = 0.15                        # trailing-egde thickness to throat opening ratio [-]
...
# compute cycle
if design_tool_version == 'losses_with_limits':
...
print("Total stage efficiency: ", round(stator.eta*rotor.eta,2))
inputs = [GR, psi, DeltaH_prod]
# tab.print_mattignly(one, two, thr, inputs)
