...
def print_mattignly_latex(one,two,thr):
    datamat = {'Stage':    ['$T_0$','$T$','$P_0$','$P$','$M$','$v$','$w$','$alpha$','$beta$'],
            ...
            'Unit':     ['K','K','Pa','Pa','-','$m/s$','$m/s$','deg','deg'],
             }
...
def print_mattignly_terminal(one,two,thr):
    datamat = {'Stage':    ['T0','T','P0','P','M','v','w','alpha','beta'],
            'One':      [round(one.T0,2), round(one.T,2), round(one.P0/1000,2), round(one.P/1000,2), round(one.vel.M,2), round(one.vel.V,2), '-', round(np.degrees(one.alpha),2), '-'],
            'Two':      [round(two.T0,2), round(two.T,2), round(two.P0/1000,2), round(two.P/1000,2), round(two.vel.M,2), round(two.vel.V,2), '-', round(np.degrees(two.alpha),2), '-'],
            'Two r':    [round(two.T0r,2), round(two.T,2), round(two.P0r/1000,2), round(two.P/1000,2), round(two.vel.Mr,2), '-', round(two.vel.W,2), '-', round(np.degrees(two.beta),2)],
            'Thr r':    [round(thr.T0r,2), round(thr.T,2), round(thr.P0r/1000,2), round(thr.P/1000,2), round(thr.vel.Mr,2), '-', round(thr.vel.W,2), '-', round(np.degrees(thr.beta),2)],
            'Thr':      [round(thr.T0,2), round(thr.T,2), round(thr.P0/1000,2), round(thr.P/1000,2), round(thr.vel.M,2), round(thr.vel.V,2), '-', round(thr.alpha,2), '-'],
            'Unit':     ['K','K','kPa','kPa','-','m/s','m/s','deg','deg'],
             }
    dfmat = pd.DataFrame(datamat)
    dfmat = dfmat.round(2)
...
def print_inputs_latex(one, thr, inputs):
    datamat = {'Parameter':    ['$T_{01}$','$T_{03}$','$P_{01}$','$P_{03}$','$delta H$','$mdot$','$GR$','$psi$','$R_{ht}$'],
               'Value':      [round(one.T0,2), round(one.T,2), round(one.P0/1000,2), round(one.P/1000,2), inputs[0]/1e+06, inputs[1], inputs[2], inputs[3], inputs[4]],
               'Unit':     ['K','K','kPa','kPa','$MJ/kg$','$kg/s$','-','-','-'],
              }
    dfmat = pd.DataFrame(datamat)
    dfmat = dfmat.round(2)
...
def print_inputs_terminal(one, thr, inputs):
    datamat = {'Parameter':    ['T01','T03','P01','P03','Delta H','mdot','GR','psi','RHT'],
               'Value':      [round(one.T0,2), round(one.T,2), round(one.P0/1000,2), round(one.P/1000,2), inputs[0]/1e+06, inputs[1], inputs[2], inputs[3], inputs[4]],
               'Unit':     ['K','K','kPa','kPa','MJ/kg','kg/s','-','-','-'],
              }
    dfmat = pd.DataFrame(datamat)
...
def print_comparison_latex(expected, eta_t, one, two, thr):
    datamat = {'Results':     ['Design tool','Reference'],
               '$eta_{tot}$': [eta_t,expected[0]],
               '$phi$':       [thr.vel.Vx/two.vel.U,expected[1]],
               # 'RPM':         [two.vel.RPM,expected[2]],
               '$Delta T_0$': [one.T0-thr.T0,expected[3]],
               'A3':          [thr.geo.A,expected[6]],
               '$h_2$':       [two.geo.h,expected[4]],
               '$u_m$':       [two.vel.U,expected[5]],
               }
    dfmat = pd.DataFrame(datamat)
    dfmat = dfmat.round(2)
    print(dfmat.to_latex(index=False, escape=False))
    ...
    array_results = [[eta_t,expected[0]],[thr.vel.Vx/two.vel.U,expected[1]],[one.T0-thr.T0,expected[3]],[thr.geo.A,expected[6]],[two.geo.h,expected[4]],[two.vel.U,expected[5]]]
    sumt = 0

    for i in range(6):
        sumt = sumt + array_results[i][0]/array_results[i][1]

    print('Average variation: ', round(abs(1-sumt/6)*100,2),'%')


def print_comparison_terminal(expected, eta_t, one, two, thr):
    datamat = {'Results':     ['Design tool','Reference'],
               'Efficiency':  [eta_t,expected[0]],
               'Flow factor': [thr.vel.Vx/two.vel.U,expected[1]],
               # 'RPM':       [two.vel.RPM,expected[2]],
               'Delta T0':    [one.T0-thr.T0,expected[3]],
               'A3':          [thr.geo.A,expected[6]],
               'h2':         [two.geo.h,expected[4]],
               'Um':         [two.vel.U,expected[5]],
               }
    dfmat = pd.DataFrame(datamat)
    dfmat = dfmat.round(2)
    print(dfmat.to_string(index=False))
...
def print_testcase_terminal(one, two, thr, inputs, expected, etat):
    print_mattignly_terminal(one,two,thr)
    print_inputs_terminal(one, thr, inputs)
    print_comparison_terminal(expected, etat, one, two, thr)

def print_testcase_latex(one, two, thr, inputs, expected, etat):
    print_mattignly_latex(one,two,thr)
    print_inputs_latex(one, thr, inputs)
    print_comparison_latex(expected, etat, one, two, thr)

    # datamat = {'Parameter':    ['$eta_{tot}$','$phi$','RPM', '$A$', '$h$', '$u_m$'],
    #             'Reference':   [eta_t, one.vel.Vx/two.vel.U, two.vel.RPM, two.geo.A, two.geo.h, two.vel.U],
    #             'Design tool': expected,
    #             'Unit':     ['K','K','kPa','kPa','$MJ/kg$','$kg/s$','-','-','-'],
    #           }
