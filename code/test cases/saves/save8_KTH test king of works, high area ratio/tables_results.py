...
def print_comparison_terminal(expected, etat, one, two, thr, stator, rotor):
    datamat = {'Results':     ['Design tool','Reference'],
               ...
               'Flow factor': [1,1],#thr.vel.Vx/two.vel.U,expected[1]],
               'Rh3':         [thr.geo.Rh,expected[2]],
               'Omega':       [thr.vel.Omega,expected[3]],
    ...
    array_results = [[etat,expected[0]],[thr.vel.Vx/two.vel.U,expected[1]],[thr.geo.Rh,expected[2]],[thr.vel.Omega,expected[3]],[two.geo.c+thr.geo.c,expected[4]]]
    sumt = 0
...
def print_testcase_terminal(one, two, thr, inputs, expected, etat, stator, rotor):
    ...
    print_comparison_terminal(expected, etat, one, two, thr, stator, rotor)

def print_testcase_latex(one, two, thr, inputs, expected, etat, stator, rotor):
    print_mattignly_latex(one,two,thr)
    print_inputs_latex(one, thr, inputs)
    print_comparison_latex(expected, etat, one, two, thr, stator, rotor)
